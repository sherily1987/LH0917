...
- **盘面**：宽基指数、自选涨跌、策略入口
- **BTC 决策**：盯着比特币现价做纸上判断。大约每 10 秒刷新报价，价格波动超过 0.2% 或满 45 秒就重新问 Jev，你确认后才改模拟组合
...
1. 代码拉取 `BTC-USD` 的 15 分钟 K 线（失败则回退到 1 小时 / 日线）和 Yahoo 现价，计算 RSI、均线、MACD、布林、ATR，以及各策略模板的最新目标仓位。
...
5. 决策页每 10 秒拉一次现价。价格偏离上次判断超过 0.2%，或已过 45 秒，就 `refresh` 再问 Jev。Jev 仍是一次一次的判断，不是逐笔流式模型。
6. 页面画出概率条。只有你点「确认纸上买入/卖出」才会改模拟组合。
